...
class background_subtractor:
    ...
    def main(self):
        ...
        #directory 이름을 test_myimg로 설정, 이 디렉토리에 업로드한 이미지에 대한 결과를 test_myimg_results에 저장.
        #image_dir = os.path.join(os.getcwd(), 'test_data', 'test_myimg')
        image_dir = self.img_dir
        prediction_dir = self.pred_dir
        #prediction_dir = os.path.join(os.getcwd(), 'test_data', 'test_myimg' + '_results' + os.sep)
